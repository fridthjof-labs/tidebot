...
| Input | Handling |
| --- | --- |
| Webhook body | HMAC-verified before queueing; 2 MiB cap; unsigned requests cost one HMAC and nothing else |
| Webhook replay | Queue retries are coordinated by one Durable Object per `X-GitHub-Delivery`; completed IDs are suppressed for seven days |
...
- Verified deliveries run from a retrying Queue rather than inside GitHub's
  webhook deadline. Ten failed attempts move to a dead-letter queue for
  operator review; see [runtimes.md](runtimes.md).
